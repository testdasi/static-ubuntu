...
mkdir -p /config/influxdb \
    ...
    && cp --update=none /static-ubuntu/grafana-unraid-stack/influxdb/influxdb.conf /config/influxdb/
...
mkdir -p /config/loki \
    ...
    && cp --update=none /static-ubuntu/grafana-unraid-stack/loki/loki-local-config.yaml /config/loki/
...
then
    mkdir -p /config/hddtemp \
        && cp --update=none /static-ubuntu/grafana-unraid-stack/hddtemp/hddtemp.db /config/hddtemp/
    echo '[info] hddtemp fixed.'
    cp --update=none /static-ubuntu/grafana-unraid-stack/telegraf/telegraf-hddtemp.conf /config/telegraf/telegraf.conf
else
    cp --update=none /static-ubuntu/grafana-unraid-stack/telegraf/telegraf-smart.conf /config/telegraf/telegraf.conf
fi
...
mkdir -p /config/promtail \
    ...
    && cp --update=none /static-ubuntu/grafana-unraid-stack/promtail/promtail.yml /config/promtail/
...
mkdir -p /config/grafana/data/plugins \
    ...
    && cp --update=none /static-ubuntu/grafana-unraid-stack/grafana/provisioning/datasources/* /config/grafana/provisioning/datasources/ \
    && cp --update=none /static-ubuntu/grafana-unraid-stack/grafana/provisioning/dashboards/* /config/grafana/provisioning/dashboards/ \
    && cp --update=none /static-ubuntu/grafana-unraid-stack/grafana/data/dashboards/* /config/grafana/data/dashboards/ \
    && cp --update=none /static-ubuntu/grafana-unraid-stack/grafana/grafana.ini /config/grafana/
